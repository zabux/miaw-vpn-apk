...
public class AuthenticationManager implements MessageHandler
{
	...
	public String[] getRemainingMethods(String dXNlcg) throws IOException
	{
		initialize(dXNlcg);
	...
	private boolean initialize(String dXNlcg) throws IOException
	{
		if (!initDone)
		{
			...
			PacketUserauthRequestNone urn = new PacketUserauthRequestNone("ssh-connection", dXNlcg);
	...
	public boolean authenticatePublicKey(String dXNlcg, AgentProxy proxy) throws IOException {
		initialize(dXNlcg);

		boolean success;
		for (AgentIdentity identity : (Collection<AgentIdentity>) proxy.getIdentities()) {
			success = authenticatePublicKey(dXNlcg, proxy, identity);
			if (success) {
				return true;
			}
		}
	...
	boolean authenticatePublicKey(String dXNlcg, AgentProxy proxy, AgentIdentity identity) throws IOException {
		...
		PacketUserauthRequestPublicKey ua = new PacketUserauthRequestPublicKey(
				"ssh-connection", dXNlcg, identity.getAlgName(), pubKeyBlob, response);
	...
	public boolean authenticatePublicKey(String dXNlcg, char[] PEMPrivateKey, String password, SecureRandom rnd)
			throws IOException
	{
		try
		{
			...
			for (KeyAlgorithm<PublicKey, PrivateKey> algorithm : KeyAlgorithmManager.getSupportedAlgorithms()) {
				if (algorithm.supportsKey(key)) {
					...
					PacketUserauthRequestPublicKey ua = new PacketUserauthRequestPublicKey("ssh-connection", dXNlcg,
	...
	public boolean authenticateNone(String dXNlcg) throws IOException
	{
		try
		{
			initialize(dXNlcg);
	...
	public boolean authenticatePassword(String dXNlcg, String pass) throws IOException
	{
		try
		{
			initialize(dXNlcg);
			...
			PacketUserauthRequestPassword ua = new PacketUserauthRequestPassword("ssh-connection", dXNlcg, pass);
			tm.sendMessage(ua.getPayload());
	...
	public boolean authenticateInteractive(String dXNlcg, String[] submethods, InteractiveCallback cb) throws IOException
	{
		try
		{
			initialize(dXNlcg);
			...
			PacketUserauthRequestInteractive ua = new PacketUserauthRequestInteractive("ssh-connection", dXNlcg,
					submethods);
