package com.miaw.pro.tunnel.tunnel;
...
import com.miaw.pro.tunnel.config.PasswordCache;
import com.miaw.pro.tunnel.config.Settings;
import com.miaw.pro.tunnel.logger.SkStatus;
import com.miaw.pro.tunnel.tunnel.vpn.TunnelState;
import com.miaw.pro.tunnel.tunnel.vpn.TunnelVpnManager;
import com.miaw.pro.tunnel.tunnel.vpn.TunnelVpnService;
import com.miaw.pro.tunnel.tunnel.vpn.TunnelVpnSettings;
import com.miaw.pro.tunnel.tunnel.vpn.VpnUtils;
...
ServerHostKeyVerifier, DebugLogger
{
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
			...
			SkStatus.updateStateString(SkStatus.SSH_CONECTADO, "Powered By : Awy_");
			SkStatus.logInfo("<b><font color='#167BA6'>Miaw VPN Connected</font><b>");
	...
	public synchronized void closeSSH() {
		...
		if (mConnection != null) {
			SkStatus.logDebug("Miaw VPN Stopping");
			mConnection.close();
		}
	}
	...
	if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("<font color='#E70000'> Akun SSH Telah Expired atau Salah pengetikan, Hubungi Admin.</font>");
    ...
    public void connectionLost(Throwable reason)
    {
        ...
        SkStatus.logInfo("<strong>" + reason.getMessage() + "</strong>");

        if (reason.getMessage().contains(
                "Connection reset by peer")) {//renz
            SkStatus.logInfo("<strong>" + "Force Reconnect" + "</strong>");
            reconnectSSH();//force recon😅 
        }
        if (reason != null) {
            if (reason.getMessage().contains(
